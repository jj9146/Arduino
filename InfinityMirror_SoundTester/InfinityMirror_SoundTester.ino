...
void loop() 
{
    ...
    while(0 == 0)
    {
        ...
        usbDetectorLib.println("Set _cancel to False at top of 0 == 0 loop"); 
        //Check the mode
        ...
        if(_displayMode == SOUND_MODE)
        {   
            usbDetectorLib.println("_displayMode = Sound Mode"); 
            SoundAlgorithm1();
/*        
            ...
            //usbDetectorLib.HaltPrintToCOM(true); // *** Use this to instantly disable all COM messages without having to remove the from the code ***
            ...
            int SampleSize = 100; // The number of analog readings collected before they're analyzed into new "Min" and "Max" values
            ...
            //**********************************************************************************
            // Now start the *REAL* main sound loop
            while (_displayMode == SOUND_MODE) // Breaks only if _displayMode changes
            {
                ...
                {
//                    usbDetectorLib.print("Filling Buffer at ");
//                    usbDetectorLib.print(BufferCounter);
//                    usbDetectorLib.print(" with ");
//                    usbDetectorLib.println(analogPinReading); 
                    ...
                    {
                        //usbDetectorLib.print("Analog Read Buffer is full at ");
                        //usbDetectorLib.println(SampleSize); 
                        T2 = millis();
                        usbDetectorLib.println("Buffer Cycle Time: " + String(T2 - T1));
                        T1 = millis();
...
//                    usbDetectorLib.print("Min: ");
//                    usbDetectorLib.print(Min);
//                    usbDetectorLib.print(" Max: ");
//                    usbDetectorLib.print(Max);
//                    usbDetectorLib.print(" Span: ");
//                    usbDetectorLib.print(Span); 
//                    usbDetectorLib.print(" Intensity: ");
//                    usbDetectorLib.print(Intensity);
//                    usbDetectorLib.print(" IntensityMultiplier: ");
//                    usbDetectorLib.println(IntensityMultiplier);
    ...
    //      the Span could drive how fast they pixel changing happens by inversely controlling the delay
                    ...
                    // Dump the before & after Red, Blue, and Green color elements on one line
                    usbDetectorLib.print("<color> (<orig>/<adjusted>)  ");
                    usbDetectorLib.print("Red (");
                    usbDetectorLib.print(Red);
                    usbDetectorLib.print("/");
                    usbDetectorLib.print(NewRed);
                    usbDetectorLib.print(") Green (");
                    usbDetectorLib.print(Green);
                    usbDetectorLib.print("/");
                    usbDetectorLib.print(NewGreen);
                    usbDetectorLib.print(") Blue (");
                    usbDetectorLib.print(Blue);
                    usbDetectorLib.print("/");
                    usbDetectorLib.print(NewBlue);
                    usbDetectorLib.println(")");
...
//                        usbDetectorLib.print("Shifted pixel ");
//                        usbDetectorLib.print(b);
//                        usbDetectorLib.print( " of ");
//                        usbDetectorLib.print(LED_COUNT);
//                        usbDetectorLib.print( " to ");
//                        usbDetectorLib.println(b - 1);
                    }

                    // With all others shifted up, change the color of the "new" (first) pixel
                    _pixelColors[0] = strip.Color(NewRed, NewGreen, NewBlue);
//                    usbDetectorLib.println("Assigned R" + String(Red) + " G:" + String(Green) + " B:" + String(Blue) + " to new pixel"); 
...
//                    usbDetectorLib.println("Assigned all array colors to the actual Neopixels"); 
                    ...
                    Crawl += CrawlRate;
//                    usbDetectorLib.print("Incremented Crawl to "); 
//                    usbDetectorLib.println(Crawl); 
                    if(Crawl > 10)
                    {
                        Crawl = 0; // Reset the "crawl" counter
                        _ColNum1++;
                        if(_ColNum1 > 255) { _ColNum1 = 0;}
//                        usbDetectorLib.println("Crawl Triggered from " + String(_ColNum1 - 1) + " to " + String(_ColNum1)); 
                    }
...
//// *** THIS IS THE **OLD** METHOD CALLED WHEN THE INTERUPT STARTS GOING HIGH ***
//void Halt()
//{
//    if(_displayMode == SOUND_MODE)
//    {
...
//            if(_cancel)
//            {
//                usbDetectorLib.println(500);
//            }
//            else
//            {
//                usbDetectorLib.println(550);
//            }
//            if(intBtn == 0)
//            {
//                usbDetectorLib.println(300);
//            }
//            else
//            {
//                usbDetectorLib.println(250);
//            }
//        }
//        else
//        {
...
//            _cancel = true;
//            usbDetectorLib.println("Set _cancel to TRUE at end of Halt");
//            
//            delay(250);
...
void RotatePixels(int rotateDirection, long wait, int numLoops)
{
    for(uint16_t x=0; x < numLoops; x++) //Set the number of loops we'll make
    {
        ...
        for(uint16_t a = 0; a < LED_COUNT; a++) //Repeat the shifting for one full loop
        {
            ...
            {
                endValue = _pixelColors[LED_COUNT - 1];  //Save the last so it can become the first
...
uint32_t WheelWithComponents(byte WheelPos, byte & Red, byte & Green, byte & Blue) 
{
    ...
    usbDetectorLib.println("R=" + String(Red) + " G=" + String(Green) + " B=" + String(Blue));
